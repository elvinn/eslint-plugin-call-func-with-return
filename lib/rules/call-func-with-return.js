    ...
    create: function(context) {
        ...
        return {
            'MemberExpression:exit': (node) => {
                var functionName = `${node.object.name}.${node.property.name}`;
                
                if (functionNameList.indexOf(functionName) === -1) {
                    return;
                }
                ...
                if (node.parent.parent && node.parent.parent.type === 'ReturnStatement') {
                    return;
                }

                return context.report({
                    node,
                    message: functionName + ' should be called with return',
                    fix: function(fixer) {
